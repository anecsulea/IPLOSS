...
sub printHelp{
    ...
    print "\n";
    print "This script runs alignments for projection clusters. \n";
    print "\n";
    print "Options:\n";
...
print "Running Pecan alignments...\n";

my $tree="'(".$sp1.",".$sp2.");'";
	   
#####################################################################################

print "Tree for Pecan: ".$tree."\n";
...
foreach my $indexclust (keys %clusters){
    ...
    if($modulo==0 || $remainder==$run){
		...
		my $pathout=$dirout."/".$gene1.".".$gene2.".mfa.gz";
		...
		if(-e $pathout){
		    ...
		    system("java bp.pecan.Pecan -E ".$tree." -F ".$sp1." ".$sp2);
		    system("gzip output.mfa");
		    system("mv output.mfa.gz ".$pathout);
