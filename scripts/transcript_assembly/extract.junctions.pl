		 ...
		 if($freqmismatch<=$maxmismatch){
			 ...
			 foreach my $k (@positionsintrons){
				 ...
				 if(exists $refjunctions->{$chr}{$key}){
				     ...
				     $refjunctions->{$chr}{$key}={"anchorleft"=>[$anchorleft],"anchorright"=>[$anchorright],"nbmismatch"=>[$nbmismatch]};
				 }
			     }
			     else{
				 $refjunctions->{$chr}={$key=>{"anchorleft"=>[$anchorleft],"anchorright"=>[$anchorright],"nbmismatch"=>[$nbmismatch]}};
...
print $outstrand "Chr\tStart\tEnd\tStrandRead\tSpliceStrand\tSpliceSite\tNbReads\n";
...
foreach my $chr (keys %junctions){
	...
	if($maxleft>=$anchordetection && $maxright>=$anchordetection){
	    ...
	    my $nbreads=0;
	    my $nbtot=@{$junctions{$chr}{$key}{"anchorleft"}};
		...
		if($strandread eq $splicestrand){
		    ...
		    $nbwrongstrand++;
		    my @u=split("_", $key);
		    print $outstrand $chr."\t".$u[0]."\t".$u[1]."\t".$u[2]."\t". $splicestrand."\t".$splicesite."\t".$nbreads."\n";
