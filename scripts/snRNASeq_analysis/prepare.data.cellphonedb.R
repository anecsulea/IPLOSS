...
for(sp in names(samples)){
    for(sample in samples[[sp]]){
        ...
        write.table(count_norm, paste('data_for_cellphonedb/cellphonedb_count_',sp,'_',sample,'_',annot,'.txt',sep=""), sep='\t', quote=F)

        ## generating meta file
        all_meta <- so[[]]
        meta_data <- cbind(rownames(all_meta), all_meta[,c("seurat_clusters")])
        write.table(meta_data,  paste('data_for_cellphonedb/cellphonedb_meta_',sp,'_',sample,'_', annot, '.txt',sep=""), sep='\t', quote=F, row.names=F)
    }
}
