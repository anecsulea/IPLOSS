...
sub readSpliceJunctionStatistics{
    my $pathin=$_[0];
    my $wrongjunctions=$_[1];
	...
	my $wj=$s[$header{"WrongJunctions"}];

	if($wj ne "NA"){
	    $wrongjunctions->{$txid}=$wj;
	}
...
sub readTranscriptCoverage{
	...
	$txcoverage->{$txid}={"sense"=>$covsense, "antisense"=>$covantisense};
...
$parameters{"pathSpliceJunctionStatistics"}="NA";
$parameters{"pathSenseAntisenseCoverage"}="NA";
$parameters{"minSenseAntisenseRatio"}=0.05;
$parameters{"pathOutputDecision"}="NA";

my @defaultpars=("pathAnnot1", "pathAnnot2", "chrList", "pathSenseAntisenseCoverage", "minSenseAntisenseRatio", "pathSpliceJunctionStatistics", "pathOutputDecision");
...
print "Reading splice junction statistics...\n";

my %wrongjunctions;

readSpliceJunctionStatistics($parameters{"pathSpliceJunctionStatistics"}, \%wrongjunctions);

my $nbwj=keys %wrongjunctions;

print "There are ".$nbwj." transcripts with wrong strand junctions.\n";

print "Done.\n";

##############################################################

print "Reading transcript coverage...\n";

my %coverage;

readTranscriptCoverage($parameters{"pathSenseAntisenseCoverage"}, \%coverage);

my $nbtxcov=keys %coverage;

print "There are ".$nbtxcov." transcripts with sense/antisense read coverage.\n";

my $minratio=$parameters{"minSenseAntisenseRatio"}+0.0;

print "Keeping transcripts with a minimum sense/antisense ratio of ".$minratio."\n";
...
foreach my $gene (keys %genetxass){
	...
	foreach my $tx (keys %{$genetxass{$gene}}){
	    ...
	    if(exists $wrongjunctions{$tx}){
		print $output $tx."\t".$gene."\tRejected\tWrongStrandJunctions\tNA\n";
	    } else{
		my $covsense=$coverage{$tx}{"sense"};
		my $covantisense=$coverage{$tx}{"antisense"};

		if($covantisense>0){
		    my $thisratio=$covsense/$covantisense;

		    if($thisratio<$minratio){
			print $output $tx."\t".$gene."\tRejected\tSenseAntisenseRatio\tNA\n";
		    } else{
		...
		foreach my $tx (keys %{$genetxass{$gene}}){
			...
			if($identical==1){
			    print $output $tx."\t".$gene."\tRejected\tIdenticalTranscript\t".$ensgene."\n";
			     ...
			     if(exists $wrongjunctions{$tx}){
				 print $output $tx."\t".$gene."\tRejected\tWrongStrandJunctions\t".$ensgene."\n";
			     } else{
				 my $covsense=$coverage{$tx}{"sense"};
				 my $covantisense=$coverage{$tx}{"antisense"};
				 ...
				 if($covantisense>0){
				     ...
				     if($thisratio<$minratio){
					 print $output $tx."\t".$gene."\tRejected\tSenseAntisenseRatio\t".$ensgene."\n";
				     } else{
